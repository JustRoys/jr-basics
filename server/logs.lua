...
AddEventHandler("jr-basics:PlayerDied", function(msg, Weapon)
    local webhook = Config.Logs.Webhooks.DeathLog
    local title = T.Titles.Death
    local message
...
AddEventHandler('playerJoining', function(reason)
    -- Admin Log
    local webhook = Config.Logs.Webhooks.JoiningLog
    local title = T.Titles.Join
    ...
    if Config.Logs.UseModLogs then
        local webhook2 = Config.Logs.Webhooks.JoiningModLog
        local title2 = T.Titles.Join
...
AddEventHandler('playerDropped', function(reason)
    ...
    if isdead then
        -- Admin Log
        local webhook = Config.Logs.Webhooks.LeavingLog
        local title = T.Titles.Leave
        ...
        -- Mod Log
        if Config.Logs.UseModLogs then
            local webhook2 = Config.Logs.Webhooks.LeavingModLog
            local title2 = T.Titles.Leave
...
function GetPlayerDetails(source)
    ...
    if Config.Logs.Options.DiscordID then
        if ids.discord then
    ...
    if GetConvar("steam_webApiKey", "false") ~= 'false' then
        if Config.Logs.Options.SteamName then
            if ids.steam then
                _steamID ="\n**Steam ID:** `" ..ids.steam.."`"
            else
                _steamID = "\n**Steam ID:** N/A"
        ...
        if Config.Logs.Options.SteamURL then
            if ids.steam then
	...
	if Config.Logs.Options.License then
        if ids.license then
            _license ="\n**License:** `" ..ids.license.."`"
        else
            _license = "\n**License:** N/A"
    ...
    if Config.Logs.Options.IP then
        if ids.ip then
            _ip ="\n**IP:** " ..ids.ip:gsub("ip:", "")
        else
            _ip = "\n**IP:** N/A"
...
function SendWebhookMessage(webhook, title, message, text, color)
    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({
        embeds = {
            {
                ["color"] = Config.Logs.Bot.Color,
                ["author"] = {
                    ["name"] = Config.Logs.Bot.Name,
                    ["icon_url"] = Config.Logs.Bot.Logo
                },
                ["title"] = title,
                ["description"] = message,
                ["footer"] = {
                    ["text"] = ""..Config.Logs.Bot.FooterText.." " .. " • " .. os.date("%x %X %p"),
                    ["icon_url"] = Config.Logs.Bot.FooterLogo,
                },
            },
        },
        avatar_url = Config.Logs.Bot.Avatar
    }), {
